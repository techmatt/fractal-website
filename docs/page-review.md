...
| --- | --- | --- | --- | --- | --- |
| `start-here.html` | n/a, above the contents | `Start here v1.md` | none | 5 placed, 1 pending | **v1 placed 2026-09-24** (start_here_ckpt146): a page of its own at the site's root, in the rail above the contents and not one of them, so it is in no `sections.jsonl` row and carries no done marker; the front page's lead points at it, because the rail is not shown below the breakpoint. Every picture on it is new to the site, drawn by `python -m builder start` off seeds and records no other figure stands on. The seat figures are a seeded draw, and `start-pink-gallery` stands in for the gallery Matt's daughter chose; `start-video` is a pending slot for a video Matt is making. |
...
| `deep-zoom.html` | yes | `Deep zoom v3.md` | Matt's second round, applied in the master | 5 placed, 4 placeholders | **v3 placed 2026-09-23** (PLACE_deep_zoom_v3_ckpt145), Matt's second review round applied by the design session. `deep-seahorse-valley` is retired with its section; `deep-shallow-and-deep`'s shallow half is Matt's own link; `deep-descent-pairs` placed by double_descent_ckpt145. The placeholders are `deep-zoom-video` and `deep-final-colorings` (held), `deep-descent-video`, and `deep-multibrots` in §The multibrots, whose prose Matt writes later (deep_zoom_slots_ckpt145); `grep -rn figure-placeholder` is the list |
| `other-artistic-techniques.html` | no, a placeholder | none | none | none | added 2026-09-24 (sections_13_14_ckpt146): the heading and one sentence saying the section is not written yet, and no done marker. |
| `cool-math.html` | no, a placeholder | none | none | none | added 2026-09-24 (sections_13_14_ckpt146): the heading and one sentence saying the section is not written yet, and no done marker. |
...
| `other-artistic-techniques.html` | none, a placeholder | 0 | 0 | 0 | n/a, carries none |
| `cool-math.html` | none, a placeholder | 0 | 0 | 0 | n/a, carries none |
