...
| `deep-zoom.html` | yes | `Deep zoom v3.md` | Matt's second round, applied in the master | 4 placed, 3 placeholders | **v3 placed 2026-09-23** (PLACE_deep_zoom_v3_ckpt145), Matt's second review round applied by the design session. `deep-seahorse-valley` is retired with its section; `deep-shallow-and-deep`'s shallow half is Matt's own link. The placeholders are `deep-zoom-video` and `deep-final-colorings` (held) and `deep-descent-pairs`, which double_descent_ckpt145 fills; `grep -rn figure-placeholder` is the list |
