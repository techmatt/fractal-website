...
| `fractal-atlases.html` | no | none | none | 1 draft | the section's prose is not drafted; the tool page it will hang off is shipped, and `atlas-places` is a still of its plate — see below. **The stub's own intro was swept for em-dashes 2026-09-07**: a stub is prose a reader can read, and the one it carried is the reason the `dashes` check reads `<main>` where a page has no prose section |
...
registered `draft` because every count in its caption is a reading of that release. Its
