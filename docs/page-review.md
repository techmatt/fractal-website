...
mode is named nowhere, which is the ruling. **No figure carries it now.**
`wallpapers-mine` was redrawn on another visit 2026-09-09, `gallery-floors` retired
2026-09-24, and `overview-pipeline` and `gallery-output`, the last two standing on panels
rendered in it, were re-picked onto current modes of `final139_general` on 2026-09-26
(publishing_fixes_ckpt152). The name is still not in `builder/vocabulary.py`'s banned
list, because tracked text outside any figure still spells it: `builder/picks.py`'s
`MODE_WORDS`, the recovered rows of retired picks in `article/figure-recipes.jsonl`, the
Deep gallery's descent record, and the wasm crate's inflection table. Banning it is a
decision of its own.
