...
`prose\old\`, which has been empty since 2026-09-04: a superseded master is trashed in
Drive now. `Running at scale` is dead: Matt trashed both its masters and no page
answers to it. **Rendering fundamentals got its first master on 2026-09-05** (2c448d6,
`Rendering fundamentals v1.md`) and **Escape-time fractals its first on 2026-09-06**, so
neither holds its own HTML as its master now. Deep zoom and Fractal atlases have since been placed (the latter 2026-09-24), and the gallery page waits for Matt.
His first pass over the site was applied on 2026-09-01, and he has since judged every
repetition himself: no row of the figure registry carries one marked not yet judged, the
last, `judges-what-the-judge-sees`, closing 2026-09-26 as "Matt approved, minor figure."
...
mode is named nowhere, which is the ruling. **Two figures still stand on panels drawn in
it, and a reader meets them as smooth** *(Matt, 2026-09-26, publishing_revert_ckpt152)*:
its pictures are the same as smooth's, so it is not a separate mode worth naming.
`overview-pipeline`'s third-stage seat `8b27fa88` and four of `gallery-output`'s
twenty-four were drawn in it; both figures were re-picked onto `final139_general` on
2026-09-26 (publishing_fixes_ckpt152) and Matt kept the originals instead, so they are
back on their 2026-09-02 pass. `builder/picks.py`'s `MODE_WORDS` spells the mode
"smooth", which is where their alt text and provenance leads get the word, and a
record's recipe clause keeps the engine's own name because that is what draws the
picture again. (`wallpapers-mine` was redrawn on another visit 2026-09-09 and
`gallery-floors` retired 2026-09-24.) The name is not in `builder/vocabulary.py`'s banned
list, because tracked records still need it: those two figures' provenance, the recovered
recipes in `article/figure-recipes.jsonl`, `MODE_WORDS`' own key, the Deep gallery's
descent record, and the wasm crate's inflection table.
...
| `wallpaper-packs/index.html` | n/a, generated | generated | n/a | none | the builder writes it from the gallery metadata and `check`'s `pages` holds the committed bytes to what the builder produces now, so there is nothing on it to review as prose and an edit by hand is a failing check. **No pack exists yet** — `assets/images/galleries/` holds only `seated-candidates`, the explorer's Gallery tab record, whose header says `"staged": true` and so is never a pack, so `builder build` writes an index with no cover tiles on it and the page is `packs-hero`, its intro sentence and a link back. Packs wait on the full-size render |
...
its line and link. Here, and not in the tab, each plane chip carries a 64-pixel miniature of its plate beside its label *(atlas_v2_ckpt146)*; on a 360 px phone the six wrap to two rows 34 px tall, still with no horizontal scroll. The slot pictures were deployed on 2026-09-26 in 6d794f1, so the served frame shows them,
here as in the tab.
...
- **No picture a visitor takes off this site carries an embedded link** — to be decided at
  ...
  holds a permalink for 344 of its 371 rows, so most of what a stamp would say is derived
