...
| `rendering-modes.html` | yes | `Rendering modes v3.md` | none | 13 placed | **the prose pass is done**: v3 placed 2026-09-04 over v2 of the same day, and the page carries twelve per-mode figures — one 2×2 a mode, `smooth` beside the mode at two locations, off Matt's picks from the recorded gallery `20260904T023748Z` — each one directly after its mode's paragraph, plus `modes-gallery`. v3 drops the *One judge, two kinds* section and replaces the kept-out-of-tried scoreboard with a **quality distribution** read off the candidate ledger's score sidecar rather than off the label stores: see the note below. v3 also carries the retirement above. **Three terminology edits landed 2026-09-06** with the Rendering fundamentals placement, applied to the page and to v3 in place rather than as a v4: *level it* becomes *percentile-stretch it*, *the same leveling and palette machinery* becomes *the same stretching and palette machinery*, and the histogram-coloring bullet's *a cousin of the leveling step* becomes *a cousin of the percentile stretch*, its link untouched. Nothing else on the page moved: it still carries 31 em-dashes in the body and 9 across its captions |
